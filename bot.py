...
# Rollen-IDs für Berechtigungen
STEMPEL_MANAGER_ROLLE_ID = 1526202327436886108   # (aktuell ungenutzt, siehe Hinweis bei Routenwache unten)
LEITUNG_ROLLE_ID         = 1526202327483285629   # darf ALLES bearbeiten (wie Admin) + Admin-Befehle

def ist_admin_oder_leitung(interaction: discord.Interaction) -> bool:
    """True für echte Discord-Admins ODER Mitglieder mit der Leitungs-Rolle.
    Wird als Berechtigungs-Check bei allen Admin-Befehlen verwendet."""
...
def get_morgen_datum():
    ...
    return f"{WOCHENTAGE_NAMEN[morgen.weekday()]}, {morgen.strftime('%d.%m.%Y')}"

def get_heute_datum():
    now = datetime.now(TIMEZONE)
    return f"{WOCHENTAGE_NAMEN[now.weekday()]}, {now.strftime('%d.%m.%Y')}"
...
# Datumsformat TT.MM.JJJJ / TT.MM.JJ, wird sowohl bei der Aufstellung als
# auch bei Abmeldungen gebraucht.
...
def ist_abmeldung_aktiv(info):
    ...
    'aktiv' angenommen, damit nichts verloren geht."""
...
def build_embed(datum, mitglieder, eingefroren=False):
    ...
    for m in mitglieder:
        ...
        # Wer trotz Abmeldung aktiv abgestimmt hat, wird auch als das gezählt,
        # was er/sie gewählt hat — die Abmeldung blockiert das Abstimmen nicht.
    ...
    embed.add_field(name=f"Komme ({len(ja_liste)})",
                     value="\n".join(ja_liste) if ja_liste else "*Niemand*", inline=True)
    embed.add_field(name=f"Komme später ({len(spaeter_liste)})",
                     value="\n".join(spaeter_liste) if spaeter_liste else "*Niemand*", inline=True)
    embed.add_field(name=f"Komme nicht ({len(nein_liste)})",
                     value="\n".join(nein_liste) if nein_liste else "*Niemand*", inline=True)
    embed.add_field(name=f"Abgemeldet ({len(abgemeldet_liste)})",
...
async def alte_aufstellungen_aufraeumen():
    """Löscht alte Aufstellungs-Nachrichten, deren geplante Löschzeit (1h nach
    Ersetzen) erreicht ist. Das Archiv wird hiervon NIE berührt."""
...
async def setrolle(interaction: discord.Interaction, rolle: discord.Role):
    ...
    await interaction.response.send_message(
        f"✅ Rolle **{rolle.name}** wurde gesetzt.\nDiese Rolle wird bei jeder Abstimmung gepingt.",
        ephemeral=True
    )
...
async def set_aufstellung(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    save_data(data)
    await interaction.response.send_message(f"✅ Meet Up-Channel gesetzt: {channel.mention}", ephemeral=True)
...
async def set_archiv(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    save_data(data)
    await interaction.response.send_message(f"✅ Archiv-Channel gesetzt: {channel.mention}", ephemeral=True)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if rolle_id:
            ...
            if rolle and rolle not in interaction.user.roles:
                await interaction.response.send_message("Du hast keine Berechtigung zur Abmeldung.", ephemeral=True)
                return
        ...
        if bis_datum < von_datum:
            await interaction.response.send_message("❌ Das Enddatum darf nicht vor dem Startdatum liegen.", ephemeral=True)
            return
...
def build_abmeldung_liste_embed(guild):
    abmeldungen = data.get("abmeldungen", {})
    embed = discord.Embed(title="Abmeldungs-Übersicht", color=EMBED_COLOR)
...
async def set_abmeldung(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    save_data(data)
    await interaction.response.send_message(f"✅ Abmeldungs-Channel gesetzt: {channel.mention}", ephemeral=True)
...
async def set_abmeldung_liste(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    save_data(data)
    await interaction.response.send_message(f"✅ Abmeldungs-Übersicht-Channel gesetzt: {channel.mention}", ephemeral=True)
    await update_abmeldung_liste(interaction.guild)
...
async def set_abmeldung_button(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    save_data(data)
    await interaction.response.send_message(f"✅ Abmeldung-Button-Channel gesetzt: {channel.mention}", ephemeral=True)
...
async def abmeldung_button_posten(interaction: discord.Interaction):
    if not data.get("channel_abmeldung_button"):
        await interaction.response.send_message(
            "❌ Kein Channel gesetzt!\nBitte zuerst **/set_abmeldung_button #channel** benutzen.", ephemeral=True
        )
        return
    await abmeldung_button_posten_intern(interaction.guild)
...
async def abmelden(interaction: discord.Interaction, von: str, bis: str, grund: str):
    ...
    aktiv_hinweis = "" if ist_abmeldung_aktiv(data["abmeldungen"][uid]) else \
        "\nℹ️ Dein Zeitraum beginnt erst später – bis dahin wirst du weiterhin normal im Meet Up geführt und kannst abstimmen."
    await interaction.response.send_message(
        f"✅ Abmeldung eingetragen!\nVon: **{von}**\nBis: **{bis}**{aktiv_hinweis}", ephemeral=True
    )
...
async def verifizierung_posten(interaction: discord.Interaction):
    if not data.get("channel_verifizierung"):
        await interaction.response.send_message(
            "❌ Kein Channel gesetzt!\nBitte zuerst **/set_verifizierung_channel #channel** benutzen.", ephemeral=True
        )
        return
    await verifizierung_posten_intern(interaction.guild)
...
# nach außen (Embeds, Buttons, Befehle) heißt das Feature "Routenwache".
#
# ⚠️ Komplett offen: es gibt hier absichtlich KEINE Rollen-/Berechtigungs-
# einschränkung mehr. Jedes Mitglied kann sich ein-/ausstempeln UND Zeiten
# für sich selbst oder andere manuell nachtragen bzw. entfernen.
...
def build_stempel_liste_embed(guild):
    ...
    for platz, (uid, info) in enumerate(eintraege, start=1):
        ...
        zeilen.append(f"{platz}. {name} ({format_dauer(info['gesamt_sekunden'])} – {info['anzahl']} Zeiträume)")
...
# Hinweis: /stempel_posten bleibt ein Admin/Leitungs-Befehl (Setup-Befehl,
# kein Teil des eigentlichen Ein-/Ausstempelns). /zeit_hinzufuegen,
# /zeit_entfernen und /meine_zeit haben KEINE Rollen-Beschränkung mehr –
# jedes Mitglied kann sie für sich selbst oder andere benutzen.
...
async def channels_info(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(
        ...
        f"Aktive Meet Up-Tage: **{tage_text}** (Details: /aufstellungstage)\n\n"
        f"— Abmeldung —\n"
